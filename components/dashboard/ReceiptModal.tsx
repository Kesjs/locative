...
export default function ReceiptModal({ isOpen, onClose, receiptData: receiptDataProp, data }: ReceiptModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white border border-[#E8E5E0] rounded-[12px] max-w-2xl w-full p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 duration-200 text-[#0F172A] my-8">
        {/* Modal Top Actions */}
        <div className="flex items-center justify-between pb-4 border-b border-[#E8E5E0] mb-6">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider text-[#0F172A] bg-[#F3F2EE] border border-[#E8E5E0] px-2.5 py-0.5 rounded-full">
              <ShieldCheckIcon className="h-3.5 w-3.5 text-[#0F172A]" />
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handlePrint}
              className="p-1.5 rounded-[6px] border border-[#E8E5E0] text-[#64635F] hover:text-[#0F172A] hover:bg-[#FAF9F6] transition cursor-pointer"
              title="Imprimer"
            >
        ...
        <div id="printable-receipt" className="border border-[#0F172A] rounded-[8px] p-6 sm:p-8 bg-[#FAF9F6] space-y-6">
          {/* Receipt Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#E8E5E0] pb-4">
            <div>
              <div className="text-[20px] font-extrabold tracking-tight text-[#0F172A]">
                QUITTANCE DE LOYER
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px]">
            <div className="p-3 bg-white border border-[#E8E5E0] rounded-[6px]">
              ...
              <div className="font-bold text-[#0F172A]">{receiptData.ownerName}</div>
            ...
            <div className="p-3 bg-white border border-[#E8E5E0] rounded-[6px]">
              ...
              <div className="font-bold text-[#0F172A]">{receiptData.tenantName}</div>
          ...
          <div className="p-3 bg-white border border-[#E8E5E0] rounded-[6px] space-y-1 text-[13px]">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-[#0F172A]">{receiptData.propertyAddress}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#64635F]">Période concernée :</span>
              <span className="font-bold text-[#0F172A]">{receiptData.month}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#64635F]">Mode de règlement :</span>
              <span className="font-semibold text-[#0F172A] uppercase">{receiptData.channel}</span>
            </div>
          </div>

          {/* Amount Paid Box */}
          <div className="bg-[#0F172A] text-white p-4 rounded-[6px] flex items-center justify-between">
            <div>
            ...
            <div className="text-right">
              <span className="text-[11px] font-bold text-[#0F172A] bg-white px-2.5 py-1 rounded">
                PAYÉ EN INTÉGRALITÉ ✓
              </span>
          ...
          <div className="pt-2 flex items-center justify-between text-[11px] text-[#64635F] border-t border-[#E8E5E0]">
            ...
            <div className="flex items-center gap-1.5 bg-white border border-[#E8E5E0] p-1.5 rounded">
              <QrCodeIcon className="h-7 w-7 text-[#0F172A]" />
              <div className="text-[9px] font-mono leading-tight text-[#0F172A]">
