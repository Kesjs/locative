      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            transition={{ duration: 0.2 }}
            className="pointer-events-auto absolute top-20 left-4 right-4 bg-white/95 backdrop-blur-xl border border-slate-200/80 rounded-2xl shadow-xl p-4 flex flex-col gap-3 md:hidden z-50"
          >
            <div className="flex flex-col gap-1 border-b border-slate-100 pb-3">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="px-3 py-2 text-[14px] font-medium text-slate-700 hover:text-emerald-600 hover:bg-emerald-50/50 rounded-lg transition-colors"
                >
                  {link.label}
                </Link>
              ))}
              ...
              {user ? (
                ...
                <>
                  <Link
                    href="/auth/login"
                    onClick={() => setMobileMenuOpen(false)}
                    className="w-full h-11 flex items-center justify-center text-slate-700 font-semibold border border-slate-200 rounded-xl text-[14px] hover:bg-slate-50 transition-colors cursor-pointer"
                  >
                    Se connecter
                  </Link>
                  <Link
                    href="/auth/register"
                    onClick={() => setMobileMenuOpen(false)}
                    className="w-full h-11 flex items-center justify-center gap-1.5 bg-slate-900 text-white font-semibold rounded-xl text-[14px] shadow-sm hover:bg-emerald-700 transition-colors cursor-pointer"
                  >
